...
export class OutlookAuthManager {
  constructor(clientId, tenantId, options = {}) {
    this.clientId = clientId;
    this.tenantId = tenantId;
    this.tokenManager = new TokenManager(clientId, options);
  ...
  async exchangeCodeForToken(code, redirectUri = null) {
    ...
    const params = new URLSearchParams({
      ...
      code: code,
       redirect_uri: redirectUri || authConfig.oauth.redirectUri.replace(':0', String(this.lastUsedPort)),
  ...
  /**
   * Create an OAuth authorization URL for a long-running HTTP API.
   * The API stores the state in its session and sends the code back through
   * authenticateWithAuthorizationCode, while the original interactive flow
   * remains available for the MCP client and bootstrap command.
   */
  async createAuthorizationUrl(redirectUri, state, options = {}) {
    const codeVerifier = this.tokenManager.generateCodeVerifier();
    const codeChallenge = this.tokenManager.generateCodeChallenge(codeVerifier);
    await this.tokenManager.storePKCEVerifier(codeVerifier);
    ...
    authUrl.searchParams.set('scope', authConfig.oauth.scope);
    authUrl.searchParams.set('state', state);
    authUrl.searchParams.set('code_challenge', codeChallenge);
    authUrl.searchParams.set('code_challenge_method', 'S256');
    // For add-account flow, force account selection so the user can pick a different account.
    // For the initial login, use select_account as before (identical behavior for single-account users).
    authUrl.searchParams.set('prompt', options.addAccount ? 'select_account' : 'select_account');
    return authUrl.toString();
  }

  async authenticateWithAuthorizationCode(code, redirectUri) {
  ...
  setActiveConnectedAccount(connectedAccountId) {
    this.tokenManager.setConnectedAccountId(connectedAccountId);
    this.activeConnectedAccountId = connectedAccountId || null;
  }
